...
function installAndroid() {
  if (_deferredInstallPrompt) {
    ...
    _deferredInstallPrompt.userChoice.then(function(result) {
      ...
      if (result.outcome === 'accepted') closeInstallModal();
    });
  } else {
    alert('To install: tap your browser menu (three dots) and select "Add to Home Screen"');
  }
}
function installIOS() {
  closeInstallModal();
  ...
  tip.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#2b2930;color:#e6e1e3;font-family:Inter,sans-serif;font-size:14px;padding:16px 20px;border-radius:16px;z-index:10002;text-align:center;line-height:1.6;box-shadow:0 8px 32px rgba(0,0,0,0.5);max-width:300px;';
  tip.innerHTML = 'Tap the <strong>Share</strong> button <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:-3px"><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg> at the bottom of Safari, then tap <strong>"Add to Home Screen"</strong>';
  document.body.appendChild(tip);
  setTimeout(function() { tip.style.opacity = '0'; tip.style.transition = 'opacity 300ms ease'; setTimeout(function() { tip.remove(); }, 300); }, 6000);
}
